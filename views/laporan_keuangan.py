...
def calculate_closing_and_tb_after_closing(df_tb_adj):
    ...
    # FIX: Memperbaiki daftar kolom (11 elemen). Ini adalah fix untuk ValueError
...
def create_income_statement_df(df_tb_adj, Total_Revenue, Total_Expense, Net_Income):
    ...
    def get_saldo_and_sum(df, tipe_nums, saldo_col='TB ADJ Debit'):
        df_filtered = df[df['Tipe_Num'].isin(tipe_nums)]
        total = df_filtered[saldo_col].sum()
        return total, df_filtered

    # PENDAPATAN (Akun 4, 8)
    Total_4, df_4 = get_saldo_and_sum(df_is, [4], 'TB ADJ Kredit')
    Total_8, df_8 = get_saldo_and_sum(df_is, [8], 'TB ADJ Kredit')
    ...
    # HPP (Akun 5)
    Total_5, df_5 = get_saldo_and_sum(df_is, [5], 'TB ADJ Debit')
    ...
    Total_6, df_6 = get_saldo_and_sum(df_is, [6], 'TB ADJ Debit')
    ...
    Total_9, df_9 = get_saldo_and_sum(df_is, [9], 'TB ADJ Debit')
...
def create_balance_sheet_df(df_tb_adj, Modal_Akhir):
    ...
    for index, row in df_current_asset.iterrows():
        data.append([row['Nama Akun'], row['TB ADJ Debit'] - row['TB ADJ Kredit'], ''])
    ...
    for index, row in df_current_liab.iterrows():
        ...
        data.append([row['Nama Akun'], row['TB ADJ Kredit'] - row['TB ADJ Debit'], ''])

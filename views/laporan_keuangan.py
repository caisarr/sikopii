...
def calc_tb(df, coa):
    """Menghitung TB dengan logika Debit/Kredit murni berdasarkan nilai."""
    if df.empty:
        ...
        # Hitung Net: Positif = Debit, Negatif = Kredit
        tb['Net'] = tb['D'] - tb['C']
...
def report_gl(df, coa):
    ...
    # Display Saldo di kolom D/K sesuai nilainya, tapi tetap respect normal balance untuk tanda positif
...
def generate_reports():
    ...
    # Hitung TB Adjusted (Kolom Worksheet) - LOGIKA FIX: Murni matematika Debit - Kredit
    # Net Awal (D-K) + Net Adj (D-K) = Net Akhir
    # Jika Net Akhir > 0 -> Debit, Jika < 0 -> Kredit
    def calc_adj(row):
    ...
    ws[['Debit', 'Kredit']] = ws.apply(lambda x: pd.Series(calc_adj(x)), axis=1)
    
    # Siapkan DataFrame untuk kalkulasi laporan
    df_calc = ws[['Kode Akun', 'Nama Akun', 'Tipe', 'Debit', 'Kredit']].copy()
    ...
    ws_disp.rename(columns={'Debit': 'TB ADJ Debit', 'Kredit': 'TB ADJ Kredit'}, inplace=True)
    
    return {
        "JU": report_gj(df), "BB": report_gl(df, coa), "WS": ws_disp, "IS": is_df, "RE": re_df, "BS": bs_df,
...
def calculate_closing_and_reporting_data(df_tb_adj):
    ...
    # Modal awal harus diambil dari kredit neraca saldo, bukan modal akhir
    ...
    # Sesuai permintaan: Modal di WS BS tetap Modal Awal (Rp 220jt), Prive tetap di Debit
    # Tidak di-override jadi Modal Akhir di Worksheet
...
def create_balance_sheet_df(df_tb_adj, Modal_Akhir):
    ...
    for _, r in df_fa.iterrows(): 
        # Akumulasi penyusutan akan otomatis negatif karena Kredit > Debit
        val = r['Debit'] - r['Kredit'] 
        data.append([r['Nama Akun'], val, ''])
    ...
    data.append(['Ekuitas', '', ''])
    # Tampilkan Modal Akhir di Laporan Posisi Keuangan (bukan Modal Awal)
